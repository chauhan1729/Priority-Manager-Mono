...
/**
 * Phase 4 (revised cycles): pick a duration, then a focus block is dropped on the timeline starting now.
 */
export function StartCycleModal({
  ...
  isPending: boolean;
  onStart: (minutes: number, note: string) => void;
  onClose: () => void;
}) {
  const initial =
