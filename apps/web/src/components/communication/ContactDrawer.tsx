        ...
        <div className="flex items-start justify-between gap-3 px-6 py-5 border-b border-blue-100">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="font-handwriting text-2xl text-ink">{contact.full_name}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          <section className="space-y-1">
            {contact.email && (
              <div className="flex items-center gap-2 text-sm text-ink-light">
                <span className="text-[10px] font-semibold uppercase tracking-wide w-12 flex-shrink-0">Email</span>
            ...
            {contact.phone && (
              <div className="flex items-center gap-2 text-sm text-ink-light">
                <span className="text-[10px] font-semibold uppercase tracking-wide w-12 flex-shrink-0">Phone</span>
                <span>{contact.phone}</span>
              </div>
            )}
            {!contact.email && !contact.phone && (
              <p className="text-xs text-ink-light/60 italic">No contact details added.</p>
            )}
          </section>
          ...
          <section>
            ...
            {nextMeeting ? (
              ...
              >
                <p className="text-xs font-medium text-ink truncate">{nextMeeting.title}</p>
                <p className="text-[10px] text-ink-light mt-0.5">
                  {formatMeetingDate(nextMeeting.start_at)} · {formatMeetingTime(nextMeeting.start_at)}
                </p>
              </a>
            ) : (
              <div className="rounded-lg border border-blue-50 bg-blue-50/40 px-3 py-2.5 text-xs text-ink-light">
                No upcoming meetings.{" "}
                <a href="/meeting-planner" className="text-blue-600 hover:underline">
                  Schedule one.
                </a>
              </div>
            )}
                ...
                {recentMeetings.map((m) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-medium text-ink truncate">{m.title}</p>
                      <p className="text-[10px] text-ink-light">{formatMeetingDate(m.start_at)}</p>
                    </div>
                    <span className={`flex-shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${MEETING_STATUS_STYLES[m.status] ?? MEETING_STATUS_STYLES["upcoming"]!}`}>
                      {m.status}
                    </span>
                  </a>
                ))}
          ...
          <section>
            <h3 className="text-[10px] font-semibold text-ink-light uppercase tracking-wide mb-2">
              ...
              {delegatedActivities.length > 0 && (
                <span className="ml-1.5 font-normal opacity-60">{delegatedActivities.length}</span>
              )}
            </h3>
            {delegatedActivities.length > 0 ? (
              <div className="space-y-1.5">
                {delegatedActivities.map((activity) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className={`text-[11px] font-medium text-ink truncate ${activity.status === "completed" ? "line-through text-ink-light" : ""}`}>
                        {activity.title}
                      </p>
                      <p className="text-[10px] text-ink-light mt-0.5">
                        {formatActivityDate(activity.activity_date)}
                        {activity.note && <span className="ml-1.5 italic">· {activity.note}</span>}
                      </p>
                    </div>
                    <span className={`flex-shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-semibold ${ACTIVITY_STATUS_STYLES[activity.status]}`}>
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[10px] font-semibold text-ink-light uppercase tracking-wide">Note</h3>
              {noteDirty && (
                <button
            ...
            />
            {noteDirty && (
              <p className="text-[10px] text-ink-light/60 mt-1">Unsaved changes — click Save note above.</p>
            )}
          </section>
