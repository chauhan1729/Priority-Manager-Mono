...
// ---------------------------------------------------------------------------
// Shared resource fields (used in both Add and Edit forms)
// ---------------------------------------------------------------------------

interface ResourceFieldsProps {
  defaults?: ProjectResource;
}

function ResourceFields({ defaults }: ResourceFieldsProps) {
  return (
    <>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            name="status"
            defaultValue={defaults?.status ?? "needed"}
            className="w-full rounded-lg border border-blue-100 bg-white px-3 py-2 text-sm text-ink focus:border-blue-400 focus:outline-none"
          >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            step="0.01"
            placeholder="0.00"
            defaultValue={defaults?.estimated_cost ?? ""}
        ...
        <div>
          ...
          <input
            name="needed_by_date"
            type="date"
            defaultValue={defaults?.needed_by_date ?? ""}
            className="w-full rounded-lg border border-blue-100 bg-white px-3 py-2 text-sm text-ink focus:border-blue-400 focus:outline-none"
          />
        </div>
      </div>
...
function AddResourceForm({ projectId, onSuccess, onCancel }: AddResourceFormProps) {
  ...
  return (
    ...
    >
      <h4 className="text-xs font-semibold text-ink-light uppercase tracking-wide">Add Resource</h4>
      <ResourceFields />
      ...
      <div className="flex justify-end gap-2">
        ...
        <button type="submit" className="rounded-lg bg-blue-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-blue-700">Add Resource</button>
      </div>
    </form>
  );
}
...
function EditResourceForm({ resource, projectId, onSuccess, onCancel }: EditResourceFormProps) {
  const boundAction = updateResource.bind(null, resource.id, projectId);
  const [state, formAction] = useActionState<ActionResult, FormData>(boundAction, null);
  ...
  return (
    ...
    >
      <h4 className="text-xs font-semibold text-ink-light uppercase tracking-wide">Edit Resource</h4>
      <ResourceFields defaults={resource} />
      {state && "error" in state && (
        <p className="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{state.error}</p>
      )}
      <div className="flex justify-end gap-2">
        <button type="button" onClick={onCancel} className="rounded-lg px-4 py-1.5 text-sm font-medium text-ink-light hover:bg-blue-50">Cancel</button>
        <button type="submit" className="rounded-lg bg-amber-500 px-4 py-1.5 text-sm font-medium text-white hover:bg-amber-600">Save Changes</button>
      </div>
    </form>
  );
}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`rounded px-2 py-0.5 text-xs font-medium ${RESOURCE_TYPE_CLASSES[resource.resource_type]}`}>
                ...
                <div className="flex flex-shrink-0 items-center gap-2">
                  <span className={`hidden sm:inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${RESOURCE_STATUS_CLASSES[resource.status]}`}>
